...
DEFAULT_LABEL = "com.polyweather.observation-lock-execution-sampler"
...
def build_program_arguments(args: argparse.Namespace) -> list[str]:
    ...
    program = [
        ...
        str(float(args.max_staleness_minutes)),
        "--max-spread",
        str(float(args.max_spread)),
        "--min-ask-depth",
        str(float(args.min_ask_depth)),
    ]
    if args.exclude_dust:
...
def build_plist(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    stdout_path = (
        ...
        if args.stdout_path
        else repo_root / "evidence" / "logs" / "observation_lock_sampler.out.log"
    )
    stderr_path = (
        Path(args.stderr_path)
        if args.stderr_path
        else repo_root / "evidence" / "logs" / "observation_lock_sampler.err.log"
    )
    return {
        "Label": args.label,
...
def install_launchd(args: argparse.Namespace, *, run=subprocess.run) -> Dict[str, Any]:
    ...
    return {
        "schema_version": "polyweather_observation_lock_sampler_launchd_install.v1",
        "paper_only": True,
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install paper-only observation-lock execution sampler launchd job.")
    ...
    parser.add_argument("--summary-output", default="evidence/observation_lock_execution/observation_lock_execution_sampler_report.json")
    ...
    parser.add_argument("--install-report-output", default="evidence/observation_lock_sampler_launchd_install_report.json")
