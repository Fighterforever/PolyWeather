...
def uninstall_launchd(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if plist_path.exists() and not args.keep_plist:
        plist_path.unlink()
    return {
        "schema_version": "polyweather_intraday_observation_launchd_uninstall.v1",
        "paper_only": True,
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Uninstall intraday observation collector launchd job.")
