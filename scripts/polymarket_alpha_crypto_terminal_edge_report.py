...
from src.trading.polymarket_alpha.crypto_probability_model import load_jsonl  # noqa: E402
from src.trading.polymarket_alpha.crypto_terminal_probability_model import (  # noqa: E402
...
DEFAULT_REPORT = DEFAULT_ROOT / "crypto_terminal_edge_report.json"
DEFAULT_CANDIDATES = DEFAULT_ROOT / "crypto_terminal_candidates.jsonl"
DEFAULT_WATCH = DEFAULT_ROOT / "crypto_terminal_watch.jsonl"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run paper-only crypto terminal/close-price probability edge scan.")
    ...
    parser.add_argument("--fetch-orderbooks", action=argparse.BooleanOptionalAction, default=False)
...
def main(argv: list[str] | None = None) -> None:
    ...
    compact["artifact_paths"] = {
        "crypto_terminal_candidates": str(args.candidates_output),
        "crypto_terminal_watch": str(args.watch_output),
    }
    write_json(args.summary_output, compact)
    print(
        json.dumps(
            {
                "parsed_terminal_market_count": report.get("parsed_terminal_market_count"),
