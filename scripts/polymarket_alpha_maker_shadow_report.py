...
DEFAULT_OUTPUT_DIR = DEFAULT_ROOT / "maker_shadow"
DEFAULT_REPORT = DEFAULT_OUTPUT_DIR / "report.json"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run paper-only Polymarket all-market maker shadow scan.")
    parser.add_argument("--active-markets", default=str(DEFAULT_ACTIVE))
    parser.add_argument("--opportunity-scoreboard", default=str(DEFAULT_DENSITY))
...
def main(argv: list[str] | None = None) -> None:
    ...
    report = build_maker_shadow_report(
        ...
        opportunity_scoreboard=load_json(args.opportunity_scoreboard),
