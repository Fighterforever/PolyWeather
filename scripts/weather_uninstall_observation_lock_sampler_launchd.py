...
def uninstall_launchd(args: argparse.Namespace, *, run=subprocess.run) -> Dict[str, Any]:
    ...
    if plist_path.exists() and not args.keep_plist:
        ...
        removed = True
    return {
        "schema_version": "polyweather_observation_lock_sampler_launchd_uninstall.v1",
        "paper_only": True,
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Uninstall paper-only observation-lock execution sampler launchd job.")
