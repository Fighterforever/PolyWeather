...
from src.trading.polymarket_alpha.weather_lp_reward_discovery import build_weather_lp_reward_discovery, load_json, load_jsonl, write_json, write_jsonl  # noqa: E402


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--family-catalog", default="evidence/bucket_family/weather_bucket_family_catalog.json")
    parser.add_argument("--reward-metadata", default="evidence/weather_lp_rewards/reward_metadata_rows.jsonl")
...
def main(argv: list[str] | None = None) -> None:
    ...
    report = build_weather_lp_reward_discovery(
        ...
        reward_metadata_rows=load_jsonl(args.reward_metadata),
    ...
    print(json.dumps({"reward_market_count": compact.get("reward_market_count"), "reward_metadata_available_count": compact.get("reward_metadata_available_count"), "reward_metadata_missing_count": compact.get("reward_metadata_missing_count"), "live_order_path": False}, indent=2, sort_keys=True))
