...
def build_program_arguments(args: argparse.Namespace) -> list[str]:
    ...
    program = [
        ...
        str(repo_root / "scripts" / "weather_threshold_latency_execution_sampler.py"),
        "--paper-only",
        ...
        args.signal_report_output,
        "--summary-output",
        args.summary_output,
        "--max-candidates",
        str(int(args.max_candidates)),
        "--max-update-age-minutes",
        str(float(args.max_update_age_minutes)),
        "--max-spread",
        str(float(args.max_spread)),
        "--min-ask-depth",
...
def build_plist(args: argparse.Namespace) -> Dict[str, Any]:
    repo_root = Path(args.repo_root).resolve()
    stdout_path = Path(args.stdout_path) if args.stdout_path else repo_root / "evidence" / "logs" / "threshold_latency_sampler.out.log"
    stderr_path = Path(args.stderr_path) if args.stderr_path else repo_root / "evidence" / "logs" / "threshold_latency_sampler.err.log"
    return {
        "Label": args.label,
...
def install_launchd(args: argparse.Namespace, *, run=subprocess.run) -> Dict[str, Any]:
    ...
    return {
        "schema_version": "polyweather_threshold_latency_sampler_launchd_install.v1",
        "paper_only": True,
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install paper-only threshold latency sampler launchd job.")
    ...
    parser.add_argument("--orderbook-archive-dir", default="evidence/threshold_latency")
    parser.add_argument("--paper-fill-dir", default="evidence/threshold_latency")
    parser.add_argument("--signal-report-output", default="evidence/threshold_latency/threshold_latency_signal_report.json")
    parser.add_argument("--summary-output", default="evidence/threshold_latency/threshold_latency_execution_sampler_report.json")
    ...
    parser.add_argument("--install-report-output", default="evidence/threshold_latency/threshold_latency_sampler_launchd_install_report.json")
