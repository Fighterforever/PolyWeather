...
def build_threshold_latency_resolved_audits(
    *,
    ...
    for fill in fills:
        ...
        resolved = market is not None and _winning_token(market)
        payout = None
        pnl = None
        if resolved and entry_price is not None:
            payout = 1.0 if _winning_token(market) == _text(fill.get("token_id")) else 0.0
            pnl = round((payout - float(entry_price)) * 100.0, 6)
        rows.append(
            {
                "schema_version": "polyweather_threshold_latency_resolved_audit.v1",
                "fill_id": fill.get("fill_id"),
                "market_slug": fill.get("market_slug"),
                "token_id": fill.get("token_id"),
                "entry_price": entry_price,
                "resolved": bool(resolved),
                "winning_token_id": _winning_token(market or {}),
                "payout": payout,
...
def build_threshold_latency_resolved_audit_report(
    *,
    fills: Iterable[Dict[str, Any]],
    closed_markets: Iterable[Dict[str, Any]],
) -> Dict[str, Any]:
    fill_rows = [row for row in fills if isinstance(row, dict)]
    audits = build_threshold_latency_resolved_audits(fills=fill_rows, closed_markets=closed_markets)
    resolved = [row for row in audits if row.get("resolved_pnl_cents") is not None]
    return {
        "schema_version": "polyweather_threshold_latency_resolved_audit_report.v1",
        "paper_only": True,
        "counts_for_live_gate": False,
        "live_order_path": False,
