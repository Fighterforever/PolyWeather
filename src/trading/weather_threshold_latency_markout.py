...
def build_threshold_latency_markouts(
    *,
    ...
    for fill in fills:
        ...
        for horizon, seconds in horizons.items():
            ...
            if not matching:
                results.append(
                    {
                        "schema_version": "polyweather_threshold_latency_markout.v1",
            ...
            results.append(
                {
                    ...
                    "missing_snapshot_reason": None if markout is not None else "missing_exit_bid_or_entry_price",
                }
            )
    return results


def build_threshold_latency_markout_report(
    *,
    fills: Iterable[Dict[str, Any]],
    orderbook_snapshots: Iterable[Dict[str, Any]],
) -> Dict[str, Any]:
    rows = build_threshold_latency_markouts(fills=fills, orderbook_snapshots=orderbook_snapshots)
    resolved = [row for row in rows if row.get("markout_cents") is not None]
    return {
        "schema_version": "polyweather_threshold_latency_markout_report.v1",
        "paper_only": True,
        "counts_for_live_gate": False,
        "live_order_path": False,
        "fill_count": len([row for row in fills if isinstance(row, dict)]),
